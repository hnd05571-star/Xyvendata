...
ADMIN_ID = 8745088070
ADMIN_CHANNEL = "@xyvenstar"  # only for admin notifications, users never see this
REFERRAL_REWARD = 2
...
async def balance(update, context):
    ...
    user_id = query.from_user.id
    user = get_user(user_id)
    text = f"⭐ *Your Balance*\n\nCurrent Balance: {user['balance']} ⭐\nTotal Earned: {user['total_earned']} ⭐" if user else "User not found."
...
async def claim_gift(update, context):
    ...
    with get_db() as conn:
        ...
        conn.execute("INSERT INTO transactions (user_id, type, amount, description) VALUES (?, ?, ?, ?)", (user_id, 'gift_purchase', -reward['cost'], f"Purchased {reward['name']}"))
...
async def withdraw(update, context):
    ...
    if not user or user['balance'] < amount:
        ...
        return
    # Deduct and create pending withdrawal
    with get_db() as conn:
        ...
        add_transaction(user_id, 'withdrawal', -amount, f"Withdrawal: {reward_name}")
    # Notify admin
...
async def reject(update, context):
    ...
    with get_db() as conn:
        ...
        add_transaction(user_id, 'refund', amount, "Withdrawal rejected – stars refunded")
